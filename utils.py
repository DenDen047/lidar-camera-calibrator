...
# return LinearSegmentedColormap.from_list('opencv_rainbow',
# opencv_rainbow_data, resolution)
...
def traj2Fig_withgt(pred_xyz, gt_xyz, axes=[0, 1]):
    """Make `matplotlib.pyplot.figure` from the 2D plot of a given trajectory.
...
def save_checkpoint_list(
    save_path,
    state_dicts: List, file_prefixes: List,
    epoch: str = '',
    filename: str = 'checkpoint.pth.tar'
):
    """Saves a given list of model dictionaries as checkpoints.
    ...
    for (prefix, state) in zip(file_prefixes, state_dicts):
        torch.save(
            state,
            save_path / '{}_{:0>3}-{}'.format(prefix, epoch, filename))
